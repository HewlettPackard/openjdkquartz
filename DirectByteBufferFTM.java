...
// -- This file replaces DirectByteBuffer for FTM access to NVRAM -- //
...
public class DirectByteBufferFTM
    ...
    implements DirectBuffer
{
    ...
    protected static final UnsafeFTM unsafe = UnsafeFTM.getUnsafe();
        ...
        implements Runnable
    {
        ...
        private static UnsafeFTM unsafe = UnsafeFTM.getUnsafe();
        ...
        public void run() {
            ...
            unsafe.freeMemory(address,size);
            address = 0;
    ...
    // Primary constructor
    //
    public DirectByteBufferFTM(int cap) {                   
    ...
    //
    public DirectByteBufferFTM(long addr, int cap, Object ob) {
        super(-1, 0, cap, cap);
        address = addr;
        cleaner = null;
        att = ob;
    ...
    //
    private DirectByteBufferFTM(long addr, int cap) {
        super(-1, 0, cap, cap);
        address = addr;
        cleaner = null;
        att = null;
    ...
    //
    protected DirectByteBufferFTM(int cap, long addr,
                                     FileDescriptor fd,
                                     Runnable unmapper)
    {
    ...
    //
    DirectByteBufferFTM(DirectBuffer db,         // package-private
    ...
    public ByteBuffer slice() {
        ...
        int off = (pos << 0);
        assert (off >= 0);
        return new DirectByteBufferFTM(this, -1, 0, rem, rem, off);
    }

    public ByteBuffer duplicate() {
        return new DirectByteBufferFTM(this,
                                              this.markValue(),
                                              this.position(),
                                              this.limit(),
                                              this.capacity(),
    ...
    public ByteBuffer put(ByteBuffer src) {
        ...
        if (src instanceof DirectByteBufferFTM) {
            if (src == this)
                throw new IllegalArgumentException();
            DirectByteBufferFTM sb = (DirectByteBufferFTM)src;
